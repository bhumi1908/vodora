...
export type SendRecruiterPeerConnectionResult = {
  id: string;
...
export async function sendRecruiterPeerConnectionRequest(
  supabase: Supabase,
  peerRecruiterId: string,
  message?: string,
): Promise<SendRecruiterPeerConnectionResult> {
  const trimmedMessage = message?.trim();

  const { data, error } = await supabase.rpc("send_recruiter_peer_connection_request", {
