...
export type SendCandidateConnectionResult = {
  id: string;
...
export async function sendCandidateConnectionRequest(
  supabase: Supabase,
  recruiterId: string,
  message?: string,
): Promise<SendCandidateConnectionResult> {
  const trimmedMessage = message?.trim();

  const { data, error } = await supabase.rpc("send_candidate_connection_request", {
