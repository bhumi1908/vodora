...
function CandidatePeerConnectModal({
  profile,
  onClose,
  onSent,
}: {
  ...
  const [message, setMessage] = useState(
    `Hi ${getFirstName(profile.name)}, I came across your profile on Vodora and I'd like to connect with you.`,
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl">
        ...
        <div className="space-y-5 p-8">
          <div>
            <label
              htmlFor="candidate-peer-profile-connect-message"
              className="mb-1.5 block text-sm font-medium text-gray-700"
            >
              Personal message
            </label>
            <textarea
              id="candidate-peer-profile-connect-message"
              rows={5}
              value={message}
...
type CandidateProfilePeerConnectButtonProps = {
  profile: CandidateProfileData;
...
export function CandidateProfilePeerConnectButton({
  profile,
  connection,
  onConnectionChange,
  compact = false,
}: CandidateProfilePeerConnectButtonProps) {
  ...
  if (connection?.status === "pending") {
    return (
      ...
      >
        ...
        {compact
          ? "Pending"
          : connection.initiatedBy === "candidate"
            ? "Request Pending"
