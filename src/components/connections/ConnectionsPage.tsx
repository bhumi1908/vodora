...
const TABS: { id: ConnectionTab; label: string; description: string }[] = [
  {
    id: "received",
    label: "Requests",
...
export function ConnectionsPage({ role }: ConnectionsPageProps) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <div className="mb-8">
        <h1 className="mb-1 text-3xl font-semibold text-gray-900">Connections</h1>
        <p className="text-gray-500">
          Manage incoming requests, track pending invites, and view your network.
        </p>
      </div>
      ...
      {counts ? (
        <div className="mb-6 flex flex-wrap gap-3">
          <div className="rounded-xl border border-green-200 bg-green-50 px-4 py-2 text-sm">
            <span className="font-semibold text-green-800">{counts.connected}</span>
          ...
          {counts.pendingReceived > 0 ? (
            <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-2 text-sm">
              ...
              <span className="text-amber-700"> requests waiting</span>
            </div>
          ) : null}
          {counts.pendingSent > 0 ? (
            <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-sm">
              <span className="font-semibold text-blue-800">{counts.pendingSent}</span>
              <span className="text-blue-700"> pending sent</span>
            </div>
          ) : null}
        </div>
      ) : null}
      ...
      {error ? (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-8 text-center">
          <p className="text-sm font-medium text-red-900">Could not load connections</p>
