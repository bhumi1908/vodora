-- Candidate-facing recruiter directory: paginated search with aggregated stats and active roles.

CREATE INDEX IF NOT EXISTS recruiters_updated_at_idx
  ON public.recruiters (updated_at DESC);

CREATE INDEX IF NOT EXISTS job_postings_recruiter_status_published_idx
  ON public.job_postings (recruiter_id, status, published_at DESC)
