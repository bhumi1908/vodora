-- Phase 1 scalability: listing indexes + paginate-before-aggregate RPC refactors.
-- Response JSON shapes are unchanged for app compatibility.

-- ---------------------------------------------------------------------------
-- Indexes
-- ---------------------------------------------------------------------------

CREATE INDEX IF NOT EXISTS recruiter_saved_candidates_recruiter_saved_at_idx
  ON public.recruiter_saved_candidates (recruiter_id, saved_at DESC);

CREATE INDEX IF NOT EXISTS candidate_work_types_work_type_candidate_idx
  ON public.candidate_work_types (work_type_id, candidate_id);

CREATE INDEX IF NOT EXISTS candidates_visible_category_updated_idx
  ON public.candidates (industry_category_id, updated_at DESC)
  WHERE visibility IN ('recruiters_only', 'public');

CREATE INDEX IF NOT EXISTS candidates_visible_availability_status_updated_idx
  ON public.candidates (availability_status, updated_at DESC)
  WHERE visibility IN ('recruiters_only', 'public');

CREATE INDEX IF NOT EXISTS candidates_visible_experience_updated_idx
  ON public.candidates (total_years_experience, updated_at DESC)
  WHERE visibility IN ('recruiters_only', 'public');

-- ---------------------------------------------------------------------------
-- search_recruiter_candidates: paginate before work_types/skills/is_saved agg
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- get_recruiter_dashboard_candidates: paginate before aggregate
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- get_recruiter_saved_candidates: paginate before aggregate
-- ---------------------------------------------------------------------------
